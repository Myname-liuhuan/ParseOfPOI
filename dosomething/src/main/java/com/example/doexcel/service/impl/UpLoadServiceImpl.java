...
import com.example.doexcel.utils.ExcelOperationXlsx;
...
public class UpLoadServiceImpl implements UpLoadService {
	...
	public Map<String, Object> parseExcel(MultipartFile file, String tableName) throws Exception{
		if(file == null || tableName == null){
			return  Encapsulation.toMap(0,"传入的参数不能为空");
		}
		...
		if (filename.length() > fileNameMinLength){
			...
			if(".xls".equals(suffixName)){
				...
				return Encapsulation.toMap(1,s1 + "\n" + s2);
		...
		return Encapsulation.toMap(0,"解析excel出错");
	}
